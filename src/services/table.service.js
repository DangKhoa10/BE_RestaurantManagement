...
class TableService {
  ...
  static getTableMatchTimeAndSeat = async ({
    SoNguoi,
    ThoiGianBatDau,
    LoaiPhieuDat,
  }) => {
  ...
  static getTableByAll = async ({
    ...
    try {
      ...
      const tables = await modelTable.find(query).populate("MaPhong");
      return {
        code: 200,
        metadata: {
          success: true,
  ...
  static getTableByRoomId = async ({ MaPhong }) => {
    try {
      const tables = await modelTable
        .find({ MaPhong: MaPhong })
        .populate("MaPhong");
      return {
        code: 200,
        metadata: {
